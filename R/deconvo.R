...
#' @param minCountSpot Removes genes present in this number or more of spots
...
pre_process <- function(st_data, minCountGene = 100, minCountSpot = 5, maxVol = 1){
  cat("## Before filtering \n")
  cat("## Number of genes:",nrow(st_data)," \n")
  cat("## Number of spots:",ncol(st_data)," \n")
  st_data <- st_data[rowSums(st_data > 0) > minCountSpot, ]
  st_data <- st_data[rowSums(st_data > 0) < maxVol*ncol(st_data), ]
  st_data <- st_data[ ,(colSums(st_data) >= minCountGene & colSums(st_data) <= 1e+06) ]
  cat("--------------------------------\n")
  cat("## After filtering \n")
  cat("## Number of genes:",nrow(st_data)," \n")
  cat("## Number of spots:",ncol(st_data)," \n")
...
build_gabriel_graph <- function(location, data_struct = "deldir", deldir.wt = F, deldir.nb = 3, dist.th = 2){
  pos <- location
  if (data_struct == "deldir"){
    delaunay <- deldir(pos[,1],pos[,2])
    ...
    final_GG <- sparseMatrix(
      i = X,
      j = Y,
      x = val,
      dims = c(max(node_ids), max(node_ids)),
      dimnames = list(rownames(location),rownames(location))
    )
  }
  else{
    dist_matrix <- as.matrix(dist(pos))
    thresh <- dist.th
    graph <- graph_from_adjacency_matrix(dist_matrix<=thresh,mode="undirected")
    graph <- simplify(graph, remove.loops = T)
    final_GG <- as_adjacency_matrix(graph)
  }
  return(final_GG)
}
...
check_graph <- function(location, data_struct = "deldir", deldir.nb = 3, dist.th = 2, vertex.size = 1, ratio = 1){
  graph <- build_gabriel_graph(location = location, data_struct = data_struct,
                               deldir.nb = deldir.nb, dist.th = dist.th) %>%
    graph_from_adjacency_matrix( mode = "undirected")
  p = plot(graph,layout=as.matrix(location, vertex.size = vertex.size, vertex.shape="circle",
...
#' @param th_vol A threshold for the volume ratio to filter genes based on expression
...
cell_groups <- function(st_data, th_vol = 0.025,  final_GG, thres){
  ...
  mask_expr <- (!th_vol > sub_volume) & (!sub_volume > 1 - th_vol)
...
get_localization_score <- function(st_data, location, th_vol, data_struct = "deldir", deldir.nb = 3, dist.th = 2){
  ...
  thres <- apply(st_data, 1, function(x){
    ...
    variance12 <- weight1[-length(weight1)] * weight2[-1] * (mean1[-length(mean1)] - mean2[2:length(mean2)])^2
  ...
  final_GG <- build_gabriel_graph(location = location, data_struct = data_struct,
                                  deldir.nb = deldir.nb, dist.th = dist.th)
  cat(paste0("## Constructing graph done ...\n"))
  data_plot <- cell_groups(st_data, th_vol, final_GG, thres)
...
get_feature_matrix <- function(st_data, markerList){
  ...
  cat(paste0("## Number of process marker genes: ",length(rownames(Xinput))," for ",numK," cell types ...\n"))
  return(Xinput)
}
...
get_deconvo_matrix <- function(Xinput, K, method = "brunet"){
  ...
  t0 <- Sys.time()
  meth <- nmfAlgorithm(method)
  NMFout <- invisible(nmf(as.matrix(Xinput_norm),ncol(K),meth))
  ...
  Q <- vapply(
    seq_len(ncol(x)),
    function(i) nnls(K, x[, i])$x,
    numeric(ncol(K)))

  colnames(Q) <- paste0("topic_", seq_len(ncol(Q)))
  rownames(Q) <- colnames(K)

  ##pheatmap::pheatmap(Q,scale = "row", display_numbers = T,cluster_rows = F,cluster_cols = F)
  h = H
  ##sweep(H,2,colSums(H),"/")
  P <- vapply(
    seq_len(ncol(h)),
...
deconvo_process <- function(st_data, location, th_vol = 0.025, data_struct = "deldir", deldir.nb = 3, dist.th = 2, markerList){
  score <- get_localization_score(st_data, location, th_vol = th_vol, data_struct = data_struct,
                                  deldir.nb = deldir.nb, dist.th = dist.th)
